...
echo -e "执行完成后如果报错请执行\033[31m source /root/.zshrc \033[0m"
echo "by lwztime"
echo "执行失败多几次大概就行了（）"
# 等待 8 秒
sleep 5
...
# 安装项目依赖
echo "下载关联脚本"
wget -O trssbot.sh https://gitee.com/lwztime/ontrss/raw/master/trssbot.sh
wget -O ntbot.sh https://gitee.com/lwztime/ontrss/raw/master/ntbot.sh
chmod 777 trssbot.sh
chmod 777 ntbot.sh
...
set_filename() {
  if [ "$OS" = "Linux" ]; then
    ...
    echo "If you think that's a bug - please file an issue to https://github.moeyy.xyz/https://github.com/Schniz/fnm/issues"
    exit 1
  fi
}
...
download_fnm() {
  if [ "$USE_HOMEBREW" = "true" ]; then
    ...
    if [ "$RELEASE" = "latest" ]; then
      URL="https://github.moeyy.xyz/https://github.com/Schniz/fnm/releases/latest/download/$FILENAME.zip"
    else
      URL="https://github.moeyy.xyz/https://github.com/Schniz/fnm/releases/download/$RELEASE/$FILENAME.zip"
    fi
